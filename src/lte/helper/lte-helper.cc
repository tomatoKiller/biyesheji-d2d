...
//-----------------------------------------------------------------------D 2 D------------------------------------------------------------
  ...
  m_d2dlinkPathlossModel = m_d2dPathlossModelFactory.Create ();
  Ptr<SpectrumPropagationLossModel> d2dSplm = m_d2dlinkPathlossModel->GetObject<SpectrumPropagationLossModel> ();
  if (d2dSplm != 0)
    {
...
TypeId LteHelper::GetTypeId (void)
{
  static TypeId
    ...
    .AddAttribute ("UsePdschForCqiGeneration",
                   ...
                   MakeBooleanChecker ())
//-----------------------------------------------------------------------D 2 D------------------------------------------------------------
    .AddAttribute ("D2dScheduler",
                   "The type of scheduler to be used for eNBs. "
                   "The allowed values for this attributes are the type names "
                   "of any class inheriting from ns3::FfMacScheduler.",
                   StringValue ("ns3::D2dCircleFfMacScheduler"),
                   MakeStringAccessor (&LteHelper::SetD2dSchedulerType,
                                       &LteHelper::GetD2dSchedulerType),
                   MakeStringChecker ())
//-------------------------------------------------------------D 2 D  O V E R-------------------------------------------------------------
...
//------------------------------------------------D 2 D -------------------------------------------------
void 
LteHelper::SetD2dSchedulerType (std::string type) 
{
  NS_LOG_FUNCTION (this << type);
  m_d2dSchedulerFactory = ObjectFactory ();
  m_d2dSchedulerFactory.SetTypeId (type);
}

std::string
LteHelper::GetD2dSchedulerType () const
{
  return m_d2dSchedulerFactory.GetTypeId ().GetName ();
} 


void 
LteHelper::SetD2dSchedulerAttribute (std::string n, const AttributeValue &v)
{
  NS_LOG_FUNCTION (this << n);
...
LteHelper::InstallSingleEnbDevice (Ptr<Node> n)
{
  ...
  Ptr<D2dCircleFfMacScheduler> d2dSched = m_d2dSchedulerFactory.Create<D2dCircleFfMacScheduler> ();
...
//-----------------------------------------------------------------------D 2 D-------------------------------------------------------
  mac->SetD2dFfMacSchedSapProvider (d2dSched->GetD2dFfMacSchedSapProvider ());
  mac->SetD2dFfMacCschedSapProvider (d2dSched->GetD2dFfMacCschedSapProvider ());

  d2dSched->SetD2dFfMacSchedSapUser (mac->GetD2dFfMacSchedSapUser ());
  d2dSched->SetD2dFfMacCschedSapUser (mac->GetD2dFfMacCschedSapUser ());
//-------------------------------------------------------------D 2 D  O V E R------------------------------------------------------
...
LteHelper::InstallSingleUeDevice (Ptr<Node> n)
{
  ...
  //-----------------------------------------------------------------------D 2 D------------------------------------------------------------
  phy->SetD2dChannel(m_d2dChannel);
  //-------------------------------------------------------------D 2 D  O V E R-------------------------------------------------------------
