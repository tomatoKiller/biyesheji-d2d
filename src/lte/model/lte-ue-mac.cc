...
//-----------------------------------------------------------------------D 2 D---------------------------------------
  else if (msg->GetMessageType () == LteControlMessage::D2D_CQI_DETECT)
  {
    Ptr<D2dCqiDetectLteControlMessage> dcdMsg = DynamicCast<D2dCqiDetectLteControlMessage> (msg);

    DetectD2dChannelQuality(dcdMsg);
  ...
  else if (msg->GetMessageType () == LteControlMessage::COM_MODE)
  {
    Ptr<ComModeLteControlMessage> modeMsg = DynamicCast<ComModeLteControlMessage> (msg);
    if (modeMsg->GetSrc() == m_rnti || modeMsg->GetDst() == m_rnti) {
      CommunicationModeChange(modeMsg);
    }
...
LteUeMac::DetectD2dChannelQuality(Ptr<D2dCqiDetectLteControlMessage> dcdMsg)
{
  ...
  if (src_rnti == m_rnti)
  {
    ...
    m_uePhySapProvider->SetD2dSrsConfigForTx(dst_rnti, dcdMsg->GetSrsConfig() );

  } 
  else if(dst_rnti == m_rnti)
  {
    m_uePhySapProvider->SetD2dSrsConfigForRx(src_rnti, dcdMsg->GetSrsConfig() );
  }
}


// void
// LteUeMac::DoReportD2dChannelQuality(uint16_t src_rnti, uint16_t dst_rnti, std::vector<double>& sinr)
// {
//   NS_LOG_FUNCTION (this);
...
LteUeMac::CommunicationModeChange(Ptr<ComModeLteControlMessage> msg)
{
  ...
  if( msg->GetMode() )
  {
    /*  enter d2d mode */
    // m_comMode = D2D_MODE;
    m_uePhySapProvider->OpenD2dMode(msg->GetSrc(), msg->GetDst() );

  } 
  else
  {
    // m_comMode = CELL_MODE;
    m_uePhySapProvider->CloseD2dMode(msg->GetSrc(), msg->GetDst() );
  }
