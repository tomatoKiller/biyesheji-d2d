...
//-----------------------------------------------------------------------D 2 D------------------------------------------------------------
  void SetD2dChannel(Ptr<SpectrumChannel> c);
  void D2dPhyPduReceived(Ptr<Packet> p);
  void DoOpenD2dMode(uint16_t src_rnti, uint16_t dst_rnti);
  void DoCloseD2dMode(uint16_t src_rnti, uint16_t dst_rnti);
  void SendD2dSrs(uint16_t dst_rnti);

  void DoSetD2dSrsConfigForTx(uint16_t dst_rnti, uint16_t d2dSrsConfig);
  void DoSetD2dSrsConfigForRx(uint16_t src_rnti, uint16_t d2dSrsConfig);

  void ReceiveD2dSrs(const SpectrumValue& sinr);
  void GenerateD2dCqiReport(const SpectrumValue& sinr, uint16_t dst_rnti);
  ...
  std::map< uint16_t, uint16_t>  m_d2dSrsOffset_tx;  //本UE作为发送方的D2D srs偏移
  std::map< uint16_t, uint16_t>  m_d2dSrsOffset_rx;  //本UE作为发送方的D2D srs偏移

  uint8_t m_d2dSrsPeriodity;
  uint8_t m_currentD2dSrsOffset;

  std::map<uint16_t, std::vector<int> > m_d2dSubChannelsForTransmission;
  std::map<uint16_t, std::vector<int> > m_d2dSubChannelsForReception;
