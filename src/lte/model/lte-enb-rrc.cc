...
LteEnbRrc::GetTypeId (void)
{
  ...
  static TypeId tid = TypeId ("ns3::LteEnbRrc")
    ...
    .AddAttribute ("RsrqFilterCoefficient",
                   ...
                   MakeUintegerChecker<uint8_t> (0))
//-----------------------------------------------------------------------D 2 D-------------------------------------------------------------------------
    .AddAttribute ("D2dCqiThreshold",
                   "the cqi threshold of d2d communication",
                   UintegerValue (5),
                   MakeUintegerAccessor (&LteEnbRrc::m_d2dCqiThreshold),
                   MakeUintegerChecker<uint8_t> ())
    .AddAttribute ("D2dResUseThreshold",
                   "d2d resource usage threshold",
                   DoubleValue (0.8),
                   MakeDoubleAccessor (&LteEnbRrc::m_d2dResUseThreshold),
    ...
    .AddAttribute ("D2dDetectPeriodicity",
                   ...
                   UintegerValue (100),
                   MakeUintegerAccessor (&LteEnbRrc::m_d2dDetectPeriodicity),
                   MakeUintegerChecker<uint8_t> ())
...
LteEnbRrc::DetectInnerCommunication()
{
  ...
  while(1)
  {
    if (iter_bef == m_ueMap.end() || iter_next == m_ueMap.end())
    {
      break;
    }

    d2dCommunicationConfig cf;
    cf.m_d2d = false;
    cf.m_d2dSrsOffset = -1;
...
LteEnbRrc::D2dCommunicationDetect()
{
  ...
  for(; iter != m_innerCom.end(); ++iter)
  {
    ...
    if (!iter->second.m_d2d && distance < m_d2dDistanceThreshold)
    {
      /* 只向不处于D2D模式，且具备D2D通信要求的那些 链路，发送CQI测量请求 */
      d2dLinkSrsConfig dlc;
      uint8_t offset;
      dlc.m_srcRnti = iter->first.first;
      dlc.m_dstRnti = iter->first.second;
      uint16_t sc = GetD2dSrsConfig(offset);
      if (sc == 50000)
      {
        /* D2D 通信对 太多，无可用偏移 */
        break;
      ...
      dlc.m_srsConfig = sc;
      linkList.push_back(dlc);
      iter->second.m_d2dSrsOffset = offset;
    }

  }

  Simulator::Schedule (MilliSeconds( m_d2dDetectPeriodicity ), &LteEnbRrc::D2dCommunicationDetect, this);

  SendD2dCqiDetectReq(linkList);
}


uint16_t
LteEnbRrc::GetD2dSrsConfig(uint8_t &offset)
{
  for(int i = 0; i < m_d2dSrsReportPeriodity; ++i)
  {
    if(!m_d2dSrsOffsetMap[i]){
      m_d2dSrsOffsetMap[i] = true;
      offset = i;
      return 317 + i;
    }
  }
