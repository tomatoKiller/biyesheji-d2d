  ...
  m_layersNum (1)
{
  ...
  m_d2dInterferenceData = CreateObject<LteInterference> ();
  m_d2dInterferenceCtrl = CreateObject<LteInterference> ();
...
LteSpectrumPhy::StartRx (Ptr<SpectrumSignalParameters> spectrumRxParams)
{
  ...
  Ptr<LteSpectrumSignalParametersD2dSrsFrame> lteD2dSrsRxParams = DynamicCast<LteSpectrumSignalParametersD2dSrsFrame> (spectrumRxParams);
  if (lteDataRxParams != 0)
    {
      ...
      //   lteDataRxParams->packetBurst->Begin()->PeekPacketTag(tag);
  ...
  else if (lteD2dSrsRxParams!=0)
    {
      m_interferenceCtrl->AddSignal (rxPsd, duration);
      StartRxD2dSrs (lteD2dSrsRxParams);
    }
  else
    {
              ...
              if (params->packetBurst)
                {
                  ...
                  {
                    LteRadioBearerTag tag;
                    (*params->packetBurst->Begin())->PeekPacketTag(tag);
                    std::map<TbId_t, tbInfo_t>::iterator iter = m_expectedTbs.begin();
                    for(; iter != m_expectedTbs.end(); ++iter)
                    {
                      if(iter->first.m_rnti == tag.GetRnti())
                      {
                        m_rxPacketBurstList.push_back (params->packetBurst);
                      }
                    }
...
bool
LteSpectrumPhy::StartTxD2dSrsFrame (uint16_t src_rnti, uint16_t dst_rnti)
{
  NS_LOG_FUNCTION (this);
  NS_LOG_LOGIC (this << " state: " << m_state);
  ...
  switch (m_state)
    ...
    case IDLE:
    {
      ...
      */
      ...
      // is done by setting the cellId parameter of 
      // LteSpectrumSignalParametersDlCtrlFrame
      ChangeState (TX_D2D_SRS);
      NS_ASSERT (m_channel);
      Ptr<LteSpectrumSignalParametersD2dSrsFrame> txParams = Create<LteSpectrumSignalParametersD2dSrsFrame> ();
      txParams->duration = UL_SRS_DURATION;
      txParams->txPhy = GetObject<SpectrumPhy> ();
      txParams->txAntenna = m_antenna;
      txParams->psd = m_txPsd;
...
void
LteSpectrumPhy::EndTxD2dSrs ()
{
  NS_LOG_FUNCTION (this);
  NS_LOG_LOGIC (this << " state: " << m_state);

  NS_ASSERT (m_state == TX_D2D_SRS);
  ChangeState (IDLE);
}
  ...
  switch (m_state)
    ...
    case RX_D2D_SRS:
      ...
      // simultaneously at the eNB
      {
        ...
        // uint16_t rnti;
        // rnti = lteD2dSrsRxParams->dst_rnti;
        // if (rnti  == m_rnti)
        //   {
            // NS_LOG_LOGIC (this << " received d2d srs  from " << lteD2dSrsRxParams->src_rnti << " to " << m_rnti);
        ...
        // else
        //   {
        //     NS_LOG_LOGIC (this << " not our d2d srs");          
        //   }
      }
      break;
...
void 
LteSpectrumPhy::SetD2dMode(bool mode)
{
  m_d2dMode = mode;
}
...
{
  m_d2dInterferenceCtrl->AddSinrChunkProcessor (p);
}
