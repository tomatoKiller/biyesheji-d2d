...
public:
  ...
  enum State
  {
    IDLE, TX_DL_CTRL, TX_DATA, TX_UL_SRS, RX_DL_CTRL, RX_DATA, RX_UL_SRS, TX_D2D_SRS, RX_D2D_SRS
  };
...
//-----------------------------------------------------------------------D 2 D------------------------------------------------------------
  bool StartTxD2dSrsFrame (uint16_t src_rnti, uint16_t dst_rnti);
  void EndTxD2dSrs ();
  void EndRxD2dSrs (uint16_t d2dSrsSrc, uint16_t d2dSrsDst);
  void StartRxD2dSrs (Ptr<LteSpectrumSignalParametersD2dSrsFrame> lteD2dSrsRxParams);
  void SetD2dMode(bool mode);
...
private:
  ...
  Ptr<LteInterference> m_d2dInterferenceCtrl;
  Ptr<LteInterference> m_d2dInterferenceData;
