...
//-----------------------------------------------------------------------D 2 D-------------------------------------------------------------------------
  bool CanUseD2dMode(uint16_t src_rnti, uint16_t dst_rnti, const std::vector<uint8_t>&  cqiList, Vector& src_pos, Vector& dst_pos);
  void DoReceiveD2dCqi(uint16_t src_rnti, uint16_t dst_rnti, const std::vector<uint8_t>&  cqi);
  void SendD2dCqiDetectReq(const std::vector<d2dLinkSrsConfig >& linkList);
  ...
  double m_uprCellThreshold;
  double m_uprD2dThreshold;
  uint8_t m_d2dDetectPeriodicity;

  uint16_t m_d2dSrsReportPeriodity;
  uint16_t m_d2dSrsOffsetCounter;

  double m_d2dDistanceThreshold;  //超过此距离，不进行D2D通信

  struct d2dCommunicationConfig
  {
    bool  m_d2d;  //是否处于d2d通信
    uint8_t m_d2dSrsOffset;
  };
