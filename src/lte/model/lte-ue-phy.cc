...
//-----------------------------------------------------------------------D 2 D------------------------------------------------------------
  virtual void OpenD2dMode(uint16_t src_rnti, uint16_t dst_rnti);

  virtual void CloseD2dMode(uint16_t src_rnti, uint16_t dst_rnti);

  // virtual void SendD2dSrs(uint16_t dst_rnti, uint8_t srsIndex);

  void SetD2dSrsConfigForTx(uint16_t dst_rnti, uint16_t d2dSrsConfig);

  void SetD2dSrsConfigForRx(uint16_t src_rnti, uint16_t d2dSrsConfig);
//-------------------------------------------------------------D 2 D  O V E R-------------------------------------------------------------
...
//-----------------------------------------------------------------------D 2 D------------------------------------------------------------
void
UeMemberLteUePhySapProvider::OpenD2dMode(uint16_t src_rnti, uint16_t dst_rnti)
{
  m_phy->DoOpenD2dMode(src_rnti, dst_rnti);
}

void
UeMemberLteUePhySapProvider::CloseD2dMode(uint16_t src_rnti, uint16_t dst_rnti)
{
  m_phy->DoCloseD2dMode(src_rnti, dst_rnti);
}

// void 
// UeMemberLteUePhySapProvider::SendD2dSrs(uint16_t dst_rnti, uint8_t srsIndex)
// {
//   NS_LOG_FUNCTION (this);
//   // m_phy->DoSendD2dSrs(dst_rnti, srsIndex);
// }

void
UeMemberLteUePhySapProvider::SetD2dSrsConfigForTx(uint16_t dst_rnti, uint16_t d2dSrsConfig)
{
  NS_LOG_FUNCTION (this);
  m_phy->DoSetD2dSrsConfigForTx(dst_rnti, d2dSrsConfig);
}

void
UeMemberLteUePhySapProvider::SetD2dSrsConfigForRx(uint16_t src_rnti, uint16_t d2dSrsConfig)
{
  NS_LOG_FUNCTION (this);
  m_phy->DoSetD2dSrsConfigForRx(src_rnti, d2dSrsConfig);
}
//-------------------------------------------------------------D 2 D  O V E R-------------------------------------------------------------
    ...
    m_rsrpSinrSampleCounter (0)
{
  ...
  // m_amc -> SetAttribute("AmcModel", "PiroEW2010");
...
//-----------------------------------------------------------------------D 2 D------------------------------------------------------------
    .AddAttribute ("D2dBandwidth",
                   "the number of d2d bandwidth rb.",
                   UintegerValue (10),
                   MakeUintegerAccessor (&LteUePhy::m_d2dBandwidth),
                   MakeUintegerChecker<uint8_t> ())
    .AddAttribute ("D2dTxPower",
                   "D2d Transmission power in dBm",
                   DoubleValue (10.0),
                   MakeDoubleAccessor (&LteUePhy::m_d2dTxPower),
                   MakeDoubleChecker<double> ())
...
//-----------------------------------------------------------------------D 2 D---------------------------------------
      else if (msg->GetMessageType () == LteControlMessage::D2D_DCI)
      {
        Ptr<D2dDciLteControlMessage> dciMsg = DynamicCast<D2dDciLteControlMessage> (msg);
        D2dDciListElement_s dci = dciMsg->GetDci();
        NS_LOG_INFO (this << " D2D DCI : " << dci.m_tx <<" ---->>> " << dci.m_rx);
        ...
        {
          m_D2dRxPhy[dci.m_tx]->AddExpectedTb(dci.m_rx, 1, 0, dci.m_mcs, d2dRb, 0, 0, 0, true);
          SetD2dSubChannelsForReception(dci.m_tx, d2dRb);
        }
        else
          continue;
        m_uePhySapUser->ReceiveLteControlMessage (msg);
...
//----------------------------------------------------D 2 D------------------------------------------------------------
          ...
          for(; itDci != m_D2dDciQueueTx.end(); ++itDci)
          {
            ...
            {
              Ptr<Packet> packet_new = (*itPa)->Copy();
              ...
              packet_new->AddPacketTag (tag1);
              // packet_new->AddPacketTag (tag2);
...
//-----------------------------------------------------------------------D 2 D------------------------------------------------------------
void
LteUePhy::SetD2dChannel(Ptr<SpectrumChannel> c)
{
  NS_LOG_FUNCTION (this);
  m_d2dChannel = c;
...
LteUePhy::DoOpenD2dMode(uint16_t src_rnti, uint16_t dst_rnti)
{
  ...
  {
    m_d2dSrsOffset_rx.erase(src_rnti);
...
LteUePhy::DoSetD2dSrsConfigForRx(uint16_t dst_rnti, uint16_t d2dSrsConfig)
{
  ...
  m_d2dNoisePsd = LteSpectrumValueHelper::CreateNoisePowerSpectralDensity (m_ulEarfcn, m_d2dBandwidth, m_noiseFigure);
  D2dRxPhy->SetNoisePowerSpectralDensity (m_d2dNoisePsd);
...
LteUePhy::SendD2dSrs(uint16_t dst_rnti)
{
  ...
  LteSpectrumValueHelper psdHelper;
  Ptr<SpectrumValue> txPsd = psdHelper.CreateTxPowerSpectralDensity (m_ulEarfcn, m_d2dBandwidth, m_d2dTxPower, d2dRb);


  m_D2dTxPhy[dst_rnti]->SetTxPowerSpectralDensity (txPsd);
  m_D2dTxPhy[dst_rnti]->StartTxD2dSrsFrame(m_rnti, dst_rnti);
...
LteUePhy::GenerateD2dCqiReport(const SpectrumValue& sinr, uint16_t dst_rnti)
{
  ...
  // m_uePhySapUser->ReportD2dChannelQuality(m_rnti, dst_rnti, ret);


  SpectrumValue newSinr = sinr;
  std::vector<int> cqi = m_amc->CreateCqiFeedbacks (newSinr, m_d2dBandwidth);
  ...
  D2dCqiListElement_s d2dcqi;
  d2dcqi.m_srcRnti = dst_rnti;
  d2dcqi.m_dstRnti = m_rnti;
  ...
  for (int i = 0; i < nLayer; i++)
    {
      if (activeSubChannels > 0)
        {
          d2dcqi.m_wbCqi.push_back ((uint16_t) cqiSum / activeSubChannels);
        }
      else
        {
          // approximate with the worst case -> CQI = 1
          d2dcqi.m_wbCqi.push_back (1);
        }
    }
...
//   // m_D2dRxPhy[tag->GetRnti()]->CalculateSinr();
// }
//-------------------------------------------------------------D 2 D  O V E R-------------------------------------------------------------
