...
//--------------------------------------------------------------D 2 D------------------------------------------------
class D2dDciLteControlMessage : public LteControlMessage
{
public:
  D2dDciLteControlMessage (void);
  virtual ~D2dDciLteControlMessage (void);

  /**
  * \brief add a DCI into the message
  * \param dci the dci
  */
  void SetDci (D2dDciListElement_s dci);

  /**
  * \brief Get dic informations
  * \return dci messages
  */
  D2dDciListElement_s GetDci (void);

private:
  D2dDciListElement_s m_dci;
};


class D2dCqiDetectLteControlMessage : public LteControlMessage
{
public:
  D2dCqiDetectLteControlMessage (void);
  virtual ~D2dCqiDetectLteControlMessage (void);

  void SetSrcDst(uint16_t srcRnti, uint16_t dstRnti);
  void SetSrsConfig (uint16_t index);
  uint16_t GetSrsConfig ();

  uint16_t GetSrc (void);
  uint16_t GetDst (void);

private:
  uint16_t m_src;
  uint16_t m_dst;

  uint16_t m_srsConfig;
};

class D2dCqiLteControlMessage : public LteControlMessage
{
public:
  D2dCqiLteControlMessage (void);
  virtual ~D2dCqiLteControlMessage (void);

  void SetD2dCqi (D2dCqiListElement_s d2dcqi);

  /**
  * \brief Get DL cqi informations
  * \return dlcqi messages
  */
  D2dCqiListElement_s GetD2dCqi (void);

private:
  D2dCqiListElement_s m_d2dCqi;
};


class ComModeLteControlMessage : public LteControlMessage
{
public:
  ComModeLteControlMessage (void);
  virtual ~ComModeLteControlMessage (void);
